...
class BpDashboardFsTile extends BaseHtmlElement
{
    protected $tag = 'div';

    protected $defaultAttributes = ['class' => 'dashboard-tile dashboard-tile-fs'];
    ...
    public function __construct(BpConfig $bp, $title, $description, $icon, $url, $urlParams = null, $attributes = null)
    {
        ...
        $tiles = Html::tag('div', ['class' => 'bp-root-tiles bp-root-tiles-fs']);
        ...
        foreach ($bp->getChildren() as $node) {
            ...
            $tiles->add(Html::tag(
                ...
                [
                    ...
                    'class' => "badge badge-fs state-{$state}",
                    'title' => $node->getAlias()
                ],
